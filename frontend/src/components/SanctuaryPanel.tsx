...
const SanctuaryPanel: React.FC = () => {
  ...
  const fetchStats = useCallback(async (r: typeof range) => {
    try {
      setLoading(true)
      const resp = await fetch(`${config.API_BASE}/api/sanctuary-statistics?range=${r}`)
    ...
    <CardFrame className="clients-card">
      <div className="clients-total-panel">
        <div className="ctp-header">
          <div className="ctp-title">Sanctuary Records</div>
        ...
        <div className="ctp-body">
            ...
            <Chart data={chartData} height={220} lineColor="#DDF581" />
          </div>
        </div>
      </div>
    </CardFrame>
