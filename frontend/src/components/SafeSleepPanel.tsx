...
const SafeSleepPanel: React.FC = () => {
  ...
  const fetchStats = useCallback(async (r: typeof range) => {
    try {
      setLoading(true)
      const resp = await fetch(`${config.API_BASE}/api/safe-sleep-statistics?range=${r}`)
    ...
    <CardFrame className="clients-card">
      <div className="clients-total-panel">
        <div className="ctp-header">
          <div className="ctp-title">Safe Sleep Records</div>
        ...
        <div className="ctp-body">
          <div className="ctp-chart">
            <Chart data={chartData} height={220} lineColor="#5668E0" />
          </div>
        </div>
      </div>
    </CardFrame>
