...
const ClientsTotalPanel: React.FC = () => {
  ...
  const fetchStats = useCallback(async (r: typeof range) => {
    try {
      setLoading(true)
      const resp = await fetch(`${config.API_BASE}/api/client-statistics?range=${r}`)
    ...
    <CardFrame className="clients-card">
      <div className="clients-total-panel">
        <div className="ctp-header">
          <div className="ctp-title">Number of clients</div>
...
export default ClientsTotalPanel
