...
const CoatCheckPanel: React.FC = () => {
  ...
  const fetchStats = useCallback(async (r: typeof range) => {
    try {
      setLoading(true)
      const resp = await fetch(`${config.API_BASE}/api/coat-check-statistics?range=${r}`)
    ...
    <CardFrame className="clients-card">
      <div className="clients-total-panel">
        <div className="ctp-header">
          <div className="ctp-title">Coat Check Records</div>
        ...
        <div className="ctp-body">
            ...
            <Chart data={chartData} height={220} lineColor="#FE2323" />
          </div>
        </div>
      </div>
      </CardFrame>
